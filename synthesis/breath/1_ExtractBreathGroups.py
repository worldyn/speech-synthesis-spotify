...
# settings
data_root = './David/more_podcast_data/resampled/' # location of wav input files '../episodes/wav_mono/' or '../../Obama/clean_renamed/'
input_root = './David/more_podcast_data/recordings/'
files = list(list_filenames(input_root, extensions='.wav', add_ext=False))
files.sort()
output_root = './David/more_podcast_data/output/'  # location of output

#%% execute process
epi = 4
input_file = files[epi] + '.wav'

#trained_model = './codes/trained_model.h5'
trained_model = './David/more_podcast_data/codes/modelMix4.h5'
nr_speakers = 1 # pretrained model is single speaker
#use_splits = True
output_prefix = files[epi][:-4]
